...
# sam2_model = SAM2VideoPredictor.from_pretrained("facebook/sam2-hiera-large").to(device)

# # Enable TF32 if on supported hardware
# if torch.cuda.is_available() and torch.cuda.get_device_properties(0).major >= 8:
#     torch.backends.cuda.matmul.allow_tf32 = True
#     torch.backends.cudnn.allow_tf32 = True

# # Streamlit app title
# st.title("Video and Image Object Detection with SAM2")
...
#     # Generate frames
#     SOURCE_FRAMES = Path("frames")
#     SOURCE_FRAMES.mkdir(parents=True, exist_ok=True)
#     frames_generator = sv.get_video_frames_generator(video_path, start=0, end=min(500, frames))
...
#             frame = sv.scale_image(frame, 0.5)
#             images_sink.save_image(frame)

#     st.write(f"Video FPS: {fps}, Total Frames: {frames}")


# def detect_objects_with_unique_labels(image_path):
#     # Load the pre-trained YOLOv8 model
#     model = YOLO("yolov8n.pt")  # Make sure you have the correct path to the YOLO model
...
#     detected_objects, results = detect_objects_with_unique_labels(temp_image_path)

#     # Display the results
#     st.write("Detected Objects with Unique Labels:")
